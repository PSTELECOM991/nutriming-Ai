      ...
      <div className="max-h-[600px] overflow-y-auto">
        <div className="p-4 space-y-4">
          {[...transactions].reverse().map((tx) => (
            <div key={tx.id} className="flex items-start gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
              <div className={`mt-1 p-2 rounded-lg ${tx.type === TransactionType.IN ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
